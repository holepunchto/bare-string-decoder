...
/** Decodes a `Buffer` into a string incrementally, buffering incomplete multi-byte sequences across calls so they are not split. */
export class StringDecoder {
  /**
   * Create a `StringDecoder` for the given `encoding`.
   * @param encoding - The character encoding the bytes are decoded from (a `BufferEncoding` such as `'utf8'` or `'utf16le'`).
   */
   ...
   * @returns The decoded string, excluding any trailing incomplete multi-byte sequence, which is buffered for the next call.
   */
   ...
   * @param buffer - A final chunk of bytes to decode together with any bytes buffered from previous calls.
